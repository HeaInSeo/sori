...
func TestError_Unwrap_NilAndNonNil(t *testing.T) {
...
func TestAllErrorSentinels(t *testing.T) {
	cases := []struct {
		...
		{validationError("op", "msg", nil), ErrValidation},
		{notFoundError("op", "msg", nil), ErrNotFound},
		{conflictError("op", "msg", nil), ErrConflict},
		{integrityError("op", "msg", nil), ErrIntegrity},
		{transportError("op", "msg", nil), ErrTransport},
	}
	for _, c := range cases {
		if !errors.Is(c.err, c.sentinel) {
			t.Errorf("errors.Is(%v, %v) returned false", c.err, c.sentinel)
		}
	}
}
